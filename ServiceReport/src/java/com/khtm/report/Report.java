...
public class Report extends HttpServlet {
            ...
            switch(action){
                case "baduser":
                    ...
                    for(ServiceUserObj suo : lst){
                        String msisdn = this.subMsisdn(suo.getMsisdn());
                        dataFile += suo.getServiceCode()+","+msisdn+","+suo.getSubStatus()
                                +","+suo.getMembershipDate()+","
                                + suo.getUnsubDate() +","+suo.getSelctedCategoryIndex()
                                +","+suo.getUnsubBy()+","+suo.getSubBy()+","+suo.getNumberType()+"\n";
                    }
                    bw.write(dataFile);
                    bw.flush();
                    bw.close();
                ...
                case "subuser":
                    ...
                    for(ServiceUserObj suo : lst_sub){
                        String msisdn = this.subMsisdn(suo.getMsisdn());
                        dataFile_sub += suo.getServiceCode()+","+msisdn+","+suo.getSubStatus()
                                +","+suo.getMembershipDate()+","
                                + suo.getUnsubDate() +","+suo.getSelctedCategoryIndex()
                                +","+suo.getUnsubBy()+","+suo.getSubBy()+","+suo.getNumberType()+"\n";
                    }
                    ...
                    FileInputStream fileStream_sub = new FileInputStream(
                            new File(folder_sub +"/report_sub_user_"+date+".csv"));
                ...
                case "unsubuser":
                    ...
                    for(ServiceUserObj suo : lst_unsub){
                        String msisdn = this.subMsisdn(suo.getMsisdn());
                        dataFile_unsub += suo.getServiceCode()+","+msisdn+","
                                +suo.getSubStatus()+","+suo.getMembershipDate()+","
                                + suo.getUnsubDate() +","+suo.getSelctedCategoryIndex()
                                +","+suo.getUnsubBy()+","+suo.getSubBy()+","+suo.getNumberType()+"\n";
                    }
                    ...
                    FileInputStream fileStream_unsub = new FileInputStream(
                            new File(folder_unsub +"/report_unsub_user_"+date+".csv"));
                ...
                case "renewal":
                    ...
                    String dataFile_renewal = "Text,Keyword,Channel,From,To,"
                            + "NotificationId,UserId,ReceiveDate\n";
                    for(NotificationObj nfo : lst_renewal){
                        String msisdn = this.subMsisdn(nfo.getFrom());
                        dataFile_renewal += nfo.getText()+","+nfo.getKeyword()+","+nfo.getChannel()
                                +","+msisdn+","+nfo.getTo()+","+nfo.getNotificationId()+","
                    ...
                    FileInputStream fileStream_renewal = new FileInputStream(
                            new File(folder_renewal +"/report_renewal_"+date+".csv"));
    ...
    public String subMsisdn(String phone){
        String[] msisdnArray = phone.split("|");
        String msisdn = "";
        for(int n = 0; n < 4; n++){
            msisdn += msisdnArray[n];
        }
        msisdn += "****";
        for(int n = 8; n < 10; n++){
            msisdn += msisdnArray[n];
